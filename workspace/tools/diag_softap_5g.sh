...
# ---------------------------------------------------------------- configuration
# Target device. Left empty, adb picks the only attached device. Override for a
# specific board (`SERIAL=<usb-serial>`) or over the network (`SERIAL=<ip>:5555`).
SERIAL="${SERIAL:-}"
SSID="CarlinkAP"
PASS="Passw0rd123"
...
# ---------------------------------------------------------------- helpers
# Empty SERIAL means "the only attached device" — `adb -s ""` is an error, so
# the -s flag has to disappear entirely rather than be passed empty.
